...
def base(request, category):
    contact_list = Posts.objects.filter(category=category).order_by('-created')
...
def politic(request):
    return base(request, 'политика')

def busines(request):
    return base(request, 'бизнес')

def sport(request):
    return base(request, 'спорт')

def fashion(request):
    return base(request, 'мода')

def travel(request):
    return base(request, 'путешествия')
